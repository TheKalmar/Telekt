    ...
    if verify_model and connection["adapter"] == "openai_compatible":
        try:
            base_url = normalize_http_base_url(
                connection["base_url"],
                allow_plain_http=connection.get("location") == "local",
        ...
        try:
            # Model endpoint was normalized immediately above.
    ...
    *,
    ...
    if mode in {"local", "hybrid"}:
        ready, detail = connection_check(
            by_id.get(settings["local_connection_id"]),
            True,
        )
        checks.append(
            {
    ...
    checks.append(
        {
            ...
            "detail": (
                cloud_detail
                if cloud_ready or cloud_required
        ...
        checks.append(
            {
                ...
                "detail": (
                    "Isolated browser runtime is online"
                    if browser_ready
    ...
    checks.append(
        {
            ...
            "detail": (
                "Isolated build and Git runtime is online"
                if execution_ready
