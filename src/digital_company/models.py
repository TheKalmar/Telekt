...
class TaskProposalDraft(BaseModel):
    ...
    expected_evidence: list[Annotated[str, Field(max_length=120)]] = Field(
        min_length=1, max_length=3
    )
    estimated_cost_eur: float = Field(default=0, ge=0)
    specialist: Literal[
        "research", "platform", "operations", "product", "development", "qa", "growth", "ceo"
    ]
    ...
    required_capabilities: list[Annotated[str, Field(max_length=80)]] = Field(
        default_factory=list, max_length=5
    )
    execution_mode: Literal["reasoning", "api", "browser", "manual", "outsourced", "build"] = (
        "reasoning"
    )
    ...
    handoff_instructions: list[Annotated[str, Field(max_length=160)]] = Field(
        default_factory=list, max_length=5
    )
    resume_evidence: list[Annotated[str, Field(max_length=160)]] = Field(
        default_factory=list, max_length=5
    )
...
class TaskProposal(TaskProposalDraft):
    ...
    def platform_access_is_explicit(self):
        ...
        if self.action == ActionType.REQUEST_PLATFORM_ACCESS and (
            not self.platform_candidate or not self.required_capabilities
        ):
            raise ValueError(
        ...
        if self.action == ActionType.REQUEST_HUMAN_HANDOFF:
            if (
                ...
                or not self.handoff_instructions
                or not self.resume_evidence
            ):
                raise ValueError(
        ...
        if self.action == ActionType.BROWSER_OPERATE:
            if self.execution_mode != "browser" or not self.handoff_url:
                raise ValueError(
                    "Browser operations require browser execution_mode and a starting URL"
                )
        ...
        if self.action in {ActionType.SAVE_CONTENT_DRAFT, ActionType.PUBLISH_CONTENT}:
            if self.execution_mode != "browser" or not self.handoff_url:
                raise ValueError(
                    "WordPress content operations require browser execution_mode and a starting URL"
                )
            if urlparse(self.handoff_url).scheme != "https":
...
class ContentPackage(BaseModel):
    ...
    internal_links: list[Annotated[str, Field(max_length=500)]] = Field(
...
class PolicyDocument(BaseModel):
    ...
    approval_actions: list[ActionType] = Field(
