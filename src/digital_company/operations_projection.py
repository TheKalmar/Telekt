...
def _project_browser_mission(events: list[dict]) -> dict | None:
    ...
    latest_start = next((
        ...
        if event["event_type"] == "browser.mission_started"
    ), None)
    if not latest_start:
        return None
    ...
    last_action = next((
        event for event in related if event["event_type"] == "browser.mission_action"
    ), None)
    last_stop = next((
        event for event in related if event["event_type"] == "browser.mission_stopped"
    ), None)
    latest_step = last_action or last_stop or {"payload": {}}
    return {
        "task_id": task_id,
        "status": (
            last_stop["payload"].get("status") if last_stop else
