...
from digital_company.agents import AgentEngine  # noqa: E402
from digital_company.models import CompanySnapshot  # noqa: E402
...
def main() -> int:
    ...
    parser.add_argument("--validate-only", action="store_true", help="Validate fixtures without model calls")
    ...
    engine = AgentEngine(mode=args.mode, local_model_name=os.getenv("LOCAL_MODEL", "deepseek-company:8b"))
    results = []
    for case in cases:
        try:
            proposal = engine.decide(CompanySnapshot.model_validate(case["snapshot"]))
            failures = grade(proposal, case["grader"])
            results.append({"id": case["id"], "passed": not failures,
                            "failures": failures, "proposal": proposal.model_dump(mode="json")})
        except Exception as exc:
            results.append({"id": case["id"], "passed": False,
                            "failures": [f"{type(exc).__name__}: {exc}"]})
    output = {"created_at": datetime.now(timezone.utc).isoformat(), "mode": args.mode,
