...
def package() -> ContentPackage:
    ...
    return ContentPackage(
        ...
        meta_description=("Neisplaćena plata zahtijeva brzu reakciju. Saznajte prava radnika, rokove i korake prema važećim propisima Republike Srpske."),
        excerpt="Praktičan pregled prava radnika kada poslodavac duguje platu.",
        categories=["Radno pravo"], tags=["plata", "radni odnos"],
        html_content=f"<h1>Neisplaćena plata</h1><p>{body}</p><h2>Šta uraditi</h2><p>Koraci.</p>",
        internal_links=["/usluge/radno-pravo"], cta="Kontaktirajte advokata radi procjene slučaja.",
        ...
        featured_image=FeaturedImageSpec(
            ...
            alt_text="Radnik pregledava obračun neisplaćene plate", filename="neisplacena-plata",
        ),
    )
