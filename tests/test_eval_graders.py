...
def test_grader_rejects_build_and_excess_cost():
    proposal = TaskProposal(
        ...
        objective="Build custom commerce",
        rationale="Fast",
        expected_evidence=["App"],
...
def test_grader_accepts_precise_handoff():
    proposal = TaskProposal(
        ...
        objective="Pass account owner checkpoint",
        rationale="Human-only",
        expected_evidence=["Account ready"],
        specialist="operations",
        execution_mode="manual",
        handoff_instructions=["Open signup and complete CAPTCHA"],
    ...
    assert (
        grade(
            proposal,
            {
                "allowed_actions": ["request_human_handoff"],
                "required_execution_mode": "manual",
                "requires_handoff_instructions": True,
                "max_cost_eur": 0,
            },
        )
        == []
