...
def test_external_work_requires_approval():
    assert (
        Governor().evaluate(proposal(ActionType.EXTERNAL_OUTREACH), 1000).outcome
        == "require_approval"
    )
...
def test_platform_evaluation_is_allowed_but_access_and_publication_need_approval():
    ...
    access = TaskProposal(
        ...
        title="Request Shopify access",
        objective="Connect the selected platform",
        rationale="Avoid custom commerce development",
        expected_evidence=["Configured access"],
        specialist="platform",
        platform_candidate="shopify",
        required_capabilities=["write_products"],
    )
    assert governor.evaluate(access, 1000).outcome == "require_approval"
    assert (
        governor.evaluate(proposal(ActionType.PUBLISH_CATALOG), 1000).outcome == "require_approval"
    )
...
def test_application_validation_still_enforces_limits_removed_from_json_schema():
    with pytest.raises(ValidationError):
        TaskProposal(
            ...
            expected_evidence=["result"],
            specialist="research",
            skill_ids=[str(index) for index in range(6)],
        )
